...
function S(e, t, n = `text/markdown`) {
  ...
  (r.href = URL.createObjectURL(new Blob([t], { type: n }))),
    (r.download = e),
    r.click(),
...
function te() {
  ...
  (0, f.useEffect)(() => {
    Promise.all([v(`providers`), v(`threads`), v(`activeId`)])
      .then(([e, n, i]) => {
        e &&
  ...
  async function De(e, t, n) {
    ...
    r =
      e.apiType === `anthropic`
        ...
        ? await fetch(x(e.baseUrl, `models/${e.model}:generateContent`), {
            ...
            body: JSON.stringify({
              systemInstruction: { parts: [{ text: n }] },
              contents: t.map((e) => ({
    ...
    if (!r.ok)
      throw Error(i?.error?.message || i?.message || `请求失败（${r.status}）`);
    let a = ``,
      o = 0,
      s = 0;
    e.apiType === `anthropic`
      ? ((a = i.content?.map((e) => e.text || ``).join(``) || ``),
        (o = i.usage?.input_tokens || 0),
        (s = i.usage?.output_tokens || 0))
      : e.apiType === `gemini`
      ? ((a =
          i.candidates?.[0]?.content?.parts
            ?.map((e) => e.text || ``)
            .join(``) || ``),
        (o = i.usageMetadata?.promptTokenCount || 0),
        (s = i.usageMetadata?.candidatesTokenCount || 0))
      : ((a = i.choices?.[0]?.message?.content || i.output_text || ``),
        (o = i.usage?.prompt_tokens || i.usage?.input_tokens || 0),
        (s = i.usage?.completion_tokens || i.usage?.output_tokens || 0));
    let c = (o / 1e6) * e.inputPrice + (s / 1e6) * e.outputPrice;
    return {
      text: a || `模型返回了空内容。`,
      usage: { input: o, output: s, cost: c },
    };
  }
  async function Oe() {
    let e = u.trim();
    ...
    try {
      if (c === `image`) {
        ...
        let e = await De(
          k,
          [...t.messages, { ...i, content: r }],
          c === `document`
            ? `你是一位专业文档编辑。请直接生成结构清晰、信息完整、可交付的中文 Markdown 文档。包含准确标题、摘要、分级标题、关键结论与下一步；不要解释你将如何写。`
            : `你是 Aurora AI，一位准确、清晰、友善的全能助手。优先使用中文回答；复杂问题先给结论，再给有条理的解释和可执行建议。`
        );
        Ee(t.id, {
          id: m(),
          role: `assistant`,
  ...
  function Pe() {
    S(
      ...
      JSON.stringify({ version: 1, providers: e, threads: i }, null, 2),
      `application/json`
    );
  }
  async function Fe(e) {
    if (e)
      try {
        ...
        if (!Array.isArray(n.threads) || !Array.isArray(n.providers))
          throw Error();
        t(n.providers), a(n.threads), Ce(`本地备份已恢复`);
      } catch {
        Ce(`备份文件无法识别`);
      }
  }
